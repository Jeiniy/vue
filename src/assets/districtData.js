...
// ✅ 2. 用來建立縣市 + 區域資料的 reactive 套件（可呼叫多次）：
export function useDistrictSelector() {
  const county = ref('')
  const district = ref('')
  const districtList = ref([])

  watch(county, (newVal) => {
    districtList.value = districtData[newVal] || []
    district.value = ''
  })

  return { county, district, districtList }
}

// ✅ 3. 小時與分鐘選單選項
export const hourOptions = Array.from({ length: 24 }, (_, i) => i < 10 ? `0${i}` : `${i}`)
export const minuteOptions = ["00", "15", "30", "45"]

// ✅ 4. 睡眠時間綁定的欄位
export const sleepHour = ref('')
export const sleepMinute = ref('')
export const wakeHour = ref('')
export const wakeMinute = ref('')
export const napHour = ref('')
export const napMinute = ref('')

// ✅ 5. 表單驗證函式（你可以依需求自行擴充）
export function validateForm() {
  if (!sleepHour.value || !wakeHour.value) {
    alert('請完整填寫睡眠時間')
    return false
  }
  return true
